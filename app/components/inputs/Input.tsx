...
const Input: React.FC<InputProps> = ({ id, label, type = 'text', disabled, formatPrice, required, register, errors }) => {
	...
	const border = errors[id] ? 'border-rose-500' : 'border-neutral-300';
	const color = errors[id] ? 'text-rose-500' : 'text-zinc-400';
	const focus = errors[id] ? 'focus:border-rose-500' : 'focus:border-black';
